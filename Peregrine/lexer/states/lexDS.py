...
@dataclass
class LexContext: # variable that will be passed into states
    current_state: str 
    ls_lvl : int = 0 # list level
    dict_lvl : int = 0 # dictionary level
    str_starter: str = '' # what starts a string (' or ")
    line: int = 1
    tab: int = 0
    state_just_changed = False # if true, run enter_state
    is_caching_token = False
    token: Token = Token()
    tokens = []

@dataclass
class LexFuncs: # for storing function that will be passed into states
    change_state: ...
    tokenize: ...
    cache_token: ...

@dataclass
class LexState: # for storing state functions
    enter_state: ...
    during_state: ...