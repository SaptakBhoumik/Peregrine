    ...
    for current_index, item in enumerate(code):
        ...
        if item=="#" and is_string==False and is_list_dictionary_string==False and is_comment==False:
            ...
            if keyword=="":
                ...
                if is_number(keyword)==TokenType.tk_int:
                    keyword+=item
                else:
                    token = create_token(
                        keyword=keyword, index=index, line=line, tab=tab, tk_type=token_type(keyword)
            ...
            if (next(current_index,code)=="*" or next(current_index,code)=="=") and (tokens[-1].tk_type==TokenType.tk_identifier or tokens[-1].tk_type==TokenType.tk_float or tokens[-1].tk_type==TokenType.tk_int or next(current_index,code)=="="):
                keyword="*"
                index=current_index
            elif keyword=="*":
                token = create_token(
