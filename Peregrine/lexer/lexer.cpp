...
LEXEME lexer(std::string src, std::string filename) {
  ...
  while (current_index < src.size()) {
    ...
    if (is_cpp == true) {
      ...
      if (item == "{" && is_list_dictionary_cpp_string == false) {
        ...
        if (third_bracket_count == 0) {
          ...
          token = token_init(statement, keyword, array, start_index,
               ...
               item == "\n" || item == "\r\n") {
      if (keyword != "") {
        token = token_init(statement, keyword,
      ...
      start_index = current_index - 1;
      keyword = item;
      token = token_init(statement, keyword, tk_at, start_index, current_index,
                         line);
    } else {
      if (keyword == "") {
        start_index = current_index;
  ...
  if (keyword != "") {
    tokens.emplace_back(token_init(
        statement, keyword, token_type(keyword, next(current_index - 1, src)),
